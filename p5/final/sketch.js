// Fall 2021 Creative Coding Final Project
// References are found at the very bottom
...
function preload() {
  sunriseImg = loadImage('sunrise.jpeg');
  //https://www.collinsdictionary.com/us/dictionary/english/sunrise
  lightningImg = loadImage('lightning.jpeg');
  //https://www.independent.co.uk/climate-change/news/lightning-length-world-record-where-longest-duration-distance-wmo-a9587101.html
  googleFont = loadFont("Raleway-VariableFont_wght.ttf");
...
function draw() {
  ...
  text("Type in your city to", 250, 230);
  text("see today's weather...", 250, 270);
  ...
  //Each weather condition has its own visual
  if (weather) {
...
function drawSun() {
  ...
  push();
    ...
    fill(255, 174, 103); //darker orange
    noStroke();
    ellipse(0, 0, 200);
    fill(255, 218, 118); //yellow
    noStroke();
...
class drawRain {
  constructor() {
    ...
    this.height = 20; //height of drizzle drop
  }
  fallingRain() {
    noStroke();
    fill(198, 215, 255); //light blue
    ellipse(this.x, this.y, 4, this.height);
    this.y = this.y + 6
    if (this.y > 500) {
      this.y = 0;
    }
...
class drawDrizzle {
  constructor() {
    this.x = random(-500, 1000); //more spread out than rain
    this.y = random(-1000,0);
    this.height = 15; //height of drizzle drop
  }
  fallingDrizzle() {
    noStroke();
    fill(198, 215, 255); //light blue
...
class drawSnow {
  ...
  update() {
    this.vel.add(this.acc); //gravity influence velocity //gets faster as it reaches the bottom
...
// ---
// Snow: https://youtu.be/cl-mHFCGzYk
// Coding Challenge #88:Snowfall by The Coding Train
